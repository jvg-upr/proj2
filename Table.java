...
public class Table extends JFrame implements ActionListener
{
    ...
    boolean turn = true; // keep track of whos turn it is, true p1, false p2
	JPanel player1;
	JPanel player2;
	JPanel deckPiles;
	JLabel deck;
	...
	public void actionPerformed(ActionEvent e)
	{
        ...
        if(cardDeck.isEmpty()) {
            Hand p1 = new Hand();
            for(Object obj : p1Hand.toArray())
                p1.addCard((Card) obj);
            Hand p2 = new Hand();
            for(Object obj : p2Hand.toArray())
                p2.addCard((Card) obj);
            int p1Val = p1.evaluateHand();
            int p2Val = p2.evaluateHand();
		...
		if((p1Deck == src && turn) || (p2Deck == src && !turn)){
		...
		if((p1Stack == src && turn) || (p2Stack == src && !turn)){
			...
			if(card != null){
				...
				if(p1Stack == src)
					...
					p2Hand.addElement(card);
                System.out.println("Added from stack: " + card);
		...
		if(p1Lay == src && turn){
			...
			if (cards != null) {
                // make sure lays are part of a set
                Character rank = null;
                for(Object obj : cards) {
                    if(rank == null) rank = ((Card) obj).getRank();
                    else if (rank != ((Card) obj).getRank()) {
                        System.out.println("Attempted to lay non-set");
                        return;
                    }
                }
